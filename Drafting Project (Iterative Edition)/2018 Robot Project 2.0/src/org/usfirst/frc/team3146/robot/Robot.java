...
public class Robot extends IterativeRobot {
	...
	//Invert the counting direction of the second encoder
	...
	double initial_value; //Measures the initial value of the imu's magnetometer upon startup
	double compass_value; //An activelty updating variable for the compass variance in auto and teleop
	double lift_value; //An actively updating variable representing the height of the lift
	double initial_lift_value; //Measures the value of the lift encoder upon startup
	double proportional_speed; //Used as a reassignable storage variable for the proportional_drive function 
	double encoder_value_average; //Averages the value of the two wheel encoders (used primarily in the proportional drive function)
	double time = 1; //The cycle number of the proportional_drive function
	...
	//Custom functions 
	public void proportional_drive(double drive_distance, double initial_value, double curve, double start_angle) {
		
		encoder_value_average = wheel_counter1.get();
		
		proportional_speed = ((drive_distance - encoder_value_average) * 0.00017);
		...
		if (proportional_speed > 0){
			if((pigeon.getFusedHeading() - (initial_value - start_angle)) > 1) { //Will fix the robots orientation in the case that it drifts, or is hit.
				robotDrive.drive(proportional_speed, curve); //Turn at .3 speed in order to compensate
				...
				robotDrive.drive(proportional_speed, 0);//Keep driving if nothing is thrown off.
			}
		}else if (proportional_speed < 0){
			if((pigeon.getFusedHeading() - (initial_value - start_angle)) > 1) { //Will fix the robots orientation in the case that it drifts, or is hit.
				robotDrive.drive(proportional_speed, -curve); //Turn at .3 speed in order to compensate
			}else if((initial_value - start_angle) - pigeon.getFusedHeading() > 1) { //Will fix the robots orientation in the case that it drifts, or is hit.
				robotDrive.drive(proportional_speed, curve); ////Turn at .3 speed in order to compensate
		...
		/*
		if(drive_distance > 0){
			if(wheel_counter1.get() < drive_distance && wheel_counter2.get() < drive_distance) {
				if((pigeon.getFusedHeading() - (initial_value - start_angle)) > 1) { //Will fix the robots orientation in the case that it drifts, or is hit.
					robotDrive.drive(proportional_speed, curve); //Turn at .3 speed in order to compensate
				}else if((initial_value - start_angle) - pigeon.getFusedHeading() > 1) { //Will fix the robots orientation in the case that it drifts, or is hit.
					robotDrive.drive(proportional_speed, -curve); ////Turn at .3 speed in order to compensate
			...
			}else if(wheel_counter1.get() > (drive_distance - 2) && wheel_counter2.get() > (drive_distance - 2)){	
			}
		}else if(drive_distance < 0){
			if(wheel_counter1.get() > drive_distance && wheel_counter2.get() > drive_distance) {
				if((pigeon.getFusedHeading() - (initial_value - start_angle)) > 1) { //Will fix the robots orientation in the case that it drifts, or is hit.
					robotDrive.drive(proportional_speed, curve); //Turn at .3 speed in order to compensate
				}else if((initial_value - start_angle) - pigeon.getFusedHeading() > 1) { //Will fix the robots orientation in the case that it drifts, or is hit.
	...
	public void snap_to_90(double potential_angle, double compass){
		if(run_init == true) {
			for(int r = 0; r < 100; r++) {
				...
				}else if(potential_angle < 0 && run_init == true){
					if(potential_angle < -360) {
						potential_angle = potential_angle + 360;
					}else {
	...
	public void auto_degree_turn( double turn_degree){
			if((initial_value - pigeon.getFusedHeading()) < (turn_degree - 4)){
				robotDrive.drive(( 1 / (initial_value - Math.abs(pigeon.getFusedHeading()) + 10) + .35), 1);
			}else if((initial_value - pigeon.getFusedHeading()) > (turn_degree + 4)){
				robotDrive.drive(( 1 / (initial_value - Math.abs(pigeon.getFusedHeading()) + 10) + .35), -1);
			}else{
				robotDrive.drive(0, 0);
			}
	...
	public void cube_shoot() {//right before this is run, set first time to 1. 
		...
		if(first_run_lift == false) {
			if(cube_shoot_timer.get() < 1 && cube_shoot_timer.get() > 0) {
				wrist.set(true);
				if(lift_value > 115 && lift_value < 125) {
					lift_screw_motor.set(0);
				}else if(lift_value > 120) {
					lift_screw_motor.set((((lift_value - 120) * -1) * 0.045) - .3);
				}else if(lift_value < 120) {
					lift_screw_motor.set(((120 - lift_value) * .045) + .4);
				}
			}else if(cube_shoot_timer.get() < 2.5 && cube_shoot_timer.get() > 2) {
				grasping_motor_left.set(-0.4);
				grasping_motor_right.set(-0.4);
		...
		//Declare updated values for compass and lift encoder
				...
				if(autoSelected == Single_Placement){
					...
					if(stationSelected == left){
						if(gameData.charAt(0) == 'L') {
							if(timer.get() < 2 && timer.get() > 0) {
								proportional_drive(-7200, initial_value, .3, 0);
							}
							...
							//drives up to switch
							if(timer.get() < 4 && timer.get() > 3) {
								proportional_drive(-6000, initial_value, .3, 90);
							}
							...
							proportional_drive(7300, initial_value, .3, 0);
						...
						if(gameData.charAt(0) == 'L'){
							...
							if(timer.get() < 1.5 && timer.get() > 0){
								if(wheel_counter1.getDistance()  > -1500){
									drive_drift_compensation(initial_value, -.6, -.3, 0); 
								}else if (wheel_counter1.getDistance() < -1700){
									drive_drift_compensation(initial_value, .3, .3, 0);
								}
							}
							
							//turn to the right
							if(timer.get() < 2.5 && timer.get() > 1.5) {
								auto_degree_turn(-45);
								wheel_counter1.reset();
							}
							if(timer.get() < 4.5 && timer.get() > 2.5){
								if(wheel_counter1.getDistance() > -3500){
									drive_drift_compensation(initial_value, -.75, -.3, -45); 
								}else if (wheel_counter1.getDistance() < -3800){
									drive_drift_compensation(initial_value, .3, .3, -45);
								}
							}
							if(timer.get() < 5.5 && timer.get() > 4.5) {
								auto_degree_turn(0);
								wheel_counter1.reset();
							}
							if(timer.get() < 6.5 && timer.get() > 5.5){
								if(wheel_counter1.getDistance() > -3200){
									drive_drift_compensation(initial_value, -.35, -.3, 0); 
								}
							}
							if(timer.get() < 8.5 && timer.get() > 6.5) {
								robotDrive.drive(-.15, 0);
								grasping_motor_left.set(-1);
								grasping_motor_right.set(-1);
							}else{
							...
							if(timer.get() < 4.5 && timer.get() > 2.5){
								if(wheel_counter1.getDistance() > -3200){
									drive_drift_compensation(initial_value, -.75, -.3, 45); 
								}else if (wheel_counter1.getDistance() < -3500){
									drive_drift_compensation(initial_value, .3, .3, 45);
								}
							}
							if(timer.get() < 5.5 && timer.get() > 4.5) {
					...
					if(stationSelected == left){
						if(gameData.charAt(0) == 'L') {
							if(timer.get() < 1.5 && timer.get() > 0) {
								if(wheel_counter1.getDistance()  > -4500){
							    	drive_drift_compensation(initial_value, -.6, -.3, 0);
								}
							}
							if(timer.get() < 3.5 && timer.get() > 1.5) {
								robotDrive.drive(-.15, 0);
								grasping_motor_left.set(-1);
								grasping_motor_right.set(-1);
							}else{
		...
		while (isOperatorControl() && isEnabled()) { // Ensures that robot is enabled and in Teleoperated mode
			...
			*/
			...
			if(stick1.getRawButton(12)){ 
				...
				//Moves arm to middle position (slightly high for driving)
				//This position will be acheived by pressing button 12 on joystick 2
				if(lift_value > 5 && lift_value < 15) {
					lift_screw_motor.set(0);
				}else if(lift_value > 10) {
					lift_screw_motor.set((((lift_value - 10) * -1) * 0.045) - .3);
				}else if(lift_value < 10) {
					lift_screw_motor.set(((10 - lift_value) * .045) + .4);
				}
